"""ABLATION: does the physics content earn its place in the surrogate?

The parametric network is anchored to reference curves at four
deterioration levels and also carries the physics losses. A referee is
entitled to ask what the physics buys that fitting four curves would not,
and the answer decides how the paper may describe itself: a
physics-regularised surrogate if the physics improves prediction away
from the anchors, and a supervised surrogate with extra steps if it does
not.

This variant answers that by training the SAME network on the SAME four
anchors with the physics losses switched off, so the only difference is
the physics. Both are then evaluated at the two withheld levels, 5 % and
25 %, which neither run has ever seen. Whichever generalises better to
those is the honest description of the method.

Set PHYSICS_OFF = True for the ablation arm.
...
Writes: runs/parametric_rho/parametric_anchored20.pt
        runs/parametric_rho/parametric_curves_anchored.json
"""
...
def main() -> None:
    ...
    for it in range(ITERS):
        ...
        if PHYSICS_OFF:
            # anchors and the arc constraint only: the arc term is kept
            # because without it there is no path parametrisation at all
            # and the comparison would not be like for like
            total = (FIXED_ARC_WEIGHT * losses["arc"]
    ...
    wall = time.time() - t0
    torch.save(last_good, out_dir / "m4_nophysics_newton.pt")
    net.load_state_dict(last_good)
    curves = eval_curves(net, prob)
    with open(out_dir / "m4_nophysics_newton_curves.json", "w") as f:
    ...
    print(f"-> {out_dir}/parametric_anchored20.pt")
    print(f"-> {out_dir}/parametric_curves_anchored.json")
