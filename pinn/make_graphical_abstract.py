...
fig = plt.figure(figsize=(13.6, 5.44))   # 5:2 per Elsevier GA spec
...
ax.text(0.5, 0.285, "four ingredients, four silent failures",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=9.4, color=INK, weight="bold", zorder=2)
...
ax.text(0.5, 0.005, "two further failures are measured, not cured:\n"
        "the boundary audit and the reference itself",
        transform=ax.transAxes, ha="center", va="top", fontsize=8.2,
        color=MUTED, zorder=2, linespacing=1.25)
ax.set_title("parametrise by path position", fontsize=10.5, weight="bold",
             loc="left", color=INK, pad=8)

# ---------------- panel c: the payoff -----------------------------------
ax = fig.add_subplot(gs[0, 2])
levels = ["0.00", "0.05", "0.10", "0.20", "0.25", "0.30"]
heldout = {"0.05", "0.25"}
cols = plt.cm.viridis(np.linspace(0.08, 0.82, len(levels)))
for k, lv in enumerate(levels):
    r = fam["curves"][lv]
    rd, rl = np.array(r["delta"]), np.array(r["lam"])
    ax.plot(rd, rl, color=cols[k], lw=1.0, alpha=0.40, zorder=2)
    n = net[lv]
    nd, nl = np.array(n["delta"]), np.array(n["lam"])
    m = nd <= rd.max()
    ls = (0, (4, 2.2)) if lv in heldout else "-"
    ax.plot(nd[m], nl[m], color=cols[k], lw=2.6, ls=ls, zorder=3,
            solid_capstyle="round")
    if lv in ("0.00", "0.10", "0.20", "0.30"):
        ax.text(rd.max() * 1.01, rl[-1], f"{float(lv)*100:.0f}%",
                color=cols[k], fontsize=8.2, va="center", weight="bold")
ax.set_ylim(0, None)
ax.text(0.035, 0.055,
        "thin: reference   thick: network   dashed: held out",
...
fig.text(0.700, 0.040, "held-out states within about 1% across seeds",
